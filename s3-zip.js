...
s3Zip.archive = function (opts, folder, filesS3, filesZip) {
  ...
  //const archive = self.archiveStreamToFile(fileStream, filesS3, filesZip)
  const archive = self.archiveStreamToMemory(fileStream, filesS3, filesZip)
  return archive
}


s3Zip.archiveStreamToMemory = function (stream, filesS3, filesZip) {
...
s3Zip.archiveStreamToFile = function (stream, filesS3, filesZip) {
  const self = this
  const folder = this.folder || ''
  if (this.registerFormat) {
    archiver.registerFormat(this.registerFormat, this.formatModule)
